...
def test_so101_arm_read_joints_with_keyword_motor_api() -> None:
    from trust_before_touch.hardware.lerobot_so101 import SO101Arm

    class FakeBus:
        def read(self, data_name: str, motor: list[str]) -> dict[str, float]:
...
def test_so101_arm_write_joints_with_keyword_motor_api(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from trust_before_touch.hardware.lerobot_so101 import SO101Arm
    ...
    class FakeBus:
        ...
        def write(self, data_name: str, values: object, motor: list[str]) -> None:
            self.register = data_name
            values_list = values if isinstance(values, FakeArray) else FakeArray([])
            self.goal = [float(v) for v in values_list.tolist()]
            self.motors = motor
